...
//        $handling_fee = $order->getBaseFoomanSurchargeAmount();
		$commodities =  array();
        $disc        = $order->getData();
        $sv_amount   = ($disc['base_sodexo_vouchers_amount']*-1);
        ...
        if ($sv_amount < 0) {
            array_push($commodities,
                array(
                    "item_id" => 'SODEXO VOUCHERS',
                    "price" => $sv_amount,
                    "quantity" => 1,
                    "item_name1" => 'Sodexo Vouchers Amount',
                    "item_name2" => 'Sodexo Vouchers Amount',
                ));
        }
...
//      	}
        ...
        Mage::log('$veritrans:'.print_r($veritrans,true),null,'vtbcaklikpay.log',true);
...
//            $orderId = str_replace($prefix,'',$orderId);
    ...
    public function notificationAction() {
        ...
        if (Mage::getStoreConfig('payment/vtbcaklikpay/api_version') == 2)
        {
            if($this->getRequest()->isPost()) {
//                $prefix = Mage::getStoreConfig('payment/vtweb/prefix_id');
